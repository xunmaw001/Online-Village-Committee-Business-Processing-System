        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '农业补贴标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.nongyebutieName" placeholder="农业补贴标题" clearable></el-input>
                     </el-form-item>
         
                     <el-form-item :label="contents.inputTitle == 1 ? '农业补贴类型' : ''">
                        <el-select v-model="searchForm.nongyebutieTypes" placeholder="请选择农业补贴类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in nongyebutieTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('nongyebutie','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/wangshangjuweihuibanlixitong/upload/nongyebutieMuBan.xls"
                        >批量导入农业补贴数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('nongyebutie','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="nongyebutieUploadSuccess"
                                :on-error="nongyebutieUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('nongyebutie','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入农业补贴数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('nongyebutie','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "nongyebutie.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nongyebutieUuidNumber"
                                   header-align="center"
                                   label="农业补贴编号">
                        <template slot-scope="scope">
                            {{scope.row.nongyebutieUuidNumber}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nongyebutieName"
                                   header-align="center"
                                   label="农业补贴标题">
                        <template slot-scope="scope">
                            {{scope.row.nongyebutieName}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="nongyebutieTypes"
                                      header-align="center"
                                      label="农业补贴类型">
                        <template slot-scope="scope">
                            {{scope.row.nongyebutieValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="nongyebutieTime"
                                   header-align="center"
                                   label="补贴时间">
                        <template slot-scope="scope">
                            {{scope.row.nongyebutieTime}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('nongyebutie','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('nongyebutie','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('nongyebutie','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //级联表下拉框搜索条件
    //当前表下拉框搜索条件
              nongyebutieTypesSelectSearch : [],
            form:{
                id : null,
                yonghuId : null,
                nongyebutieUuidNumber : null,
                nongyebutieName : null,
                nongyebutieTypes : null,
                butieJine : null,
                nongyebutieTime : null,
                nongyebutieContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '用户邮箱': 'yonghuEmail',
                //本表字段
                     '农业补贴编号': "nongyebutieUuidNumber",
                     '农业补贴标题': "nongyebutieName",
                     '农业补贴类型': "nongyebutieValue",
                     '补贴金额': "butieJine",
                     '补贴时间': "nongyebutieTime",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'nongyebutie',//当前表表名,
                        sumColum : 'nongyebutie_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'nongyebutie_name',//分组字符串字段
                        // types : 'nongyebutie_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.nongyebutieName!= '' && this.searchForm.nongyebutieName!= undefined) {
                    params['nongyebutieName'] = '%' + this.searchForm.nongyebutieName + '%'
                }
         
                if (this.searchForm.nongyebutieTypes!= '' && this.searchForm.nongyebutieTypes!= undefined) {
                    params['nongyebutieTypes'] = this.searchForm.nongyebutieTypes
                }
                                        
                params['nongyebutieDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "nongyebutie/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=nongyebutie_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.nongyebutieTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            nongyebutieUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "nongyebutie/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
